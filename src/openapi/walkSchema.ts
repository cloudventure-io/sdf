...
export const walkSchema = (
  ...
  if (schema.allOf) {
    schema.allOf.forEach((entry, index) => walkSchema(`${trace}/allOf/${index}`, entry, handler, schema))
  }
  if (schema.oneOf) {
    schema.oneOf.forEach((entry, index) => walkSchema(`${trace}/oneOf/${index}`, entry, handler, schema))
  }
  if (schema.anyOf) {
    schema.anyOf.forEach((entry, index) => walkSchema(`${trace}/anyOf/${index}`, entry, handler, schema))
  }
}
